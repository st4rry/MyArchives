...
$NMAP --open --reason -sV -A -T4 -Pn -PO -sS $SCAN_SUBNETS -R -oX scan-$DATE.xml --stylesheet "nmap.xsl" > /dev/null
...
then
    ...
    then
            ...
            echo -e "AutoNmap2 found differences in a scan for '${SCAN_SUBNETS}' since yesterday. \n\n$DIFF\n\nFull report available at $REP_DIR" | cat - > $REP_DIR/ndiffrep-$DATE.xml #mail -s "AutoNmap2" $EMAIL_RECIPIENTS
    else
...
echo -e "'date' - Converting xml to html .... \n"
xsltproc scan-$DATE.xml -o $REP_DIR/scan-$DATE.html
xsltproc $REP_DIR/ndiffrep-$DATE.xml -o $REP_DIR/ndiffrep-$DATE.html
